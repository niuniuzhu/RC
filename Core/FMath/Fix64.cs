...
namespace RC.Core.FMath
{
	...
	public partial struct Fix64 : IEquatable<Fix64>, IComparable<Fix64>
	{
		...
		public static readonly Fix64 Epsilon = ( Fix64 )( 1E-6F );
		public static readonly Fix64 One = new Fix64( ONE );
		public static readonly Fix64 Two = new Fix64( TWO );
		public static readonly Fix64 NegativeOne = new Fix64( NEGATIVE_ONE );
		...
		const long HALF = ( long )( 0.5 * ( 1L << FRACTIONAL_PLACES ) );
		const double DIVONE = 1D / ONE;
		const long ONE = 1L << FRACTIONAL_PLACES;
		const long TWO = 2L << FRACTIONAL_PLACES;
		const long NEGATIVE_ONE = -1L << FRACTIONAL_PLACES;
		...
		const int LUT_SHIFT = 20;
		const int LUT_SIZE = ( int )( PI_OVER_2 >> LUT_SHIFT );
		...
		public static Fix64 FastSin( Fix64 x )
		{
			...
			// equal to (PI_OVER_2 >> LUT_SHIFT) to use the angle to index directly into it
			var rawIndex = ( uint )( clampedL >> LUT_SHIFT );
		...
		static long ClampSinValue( long angle, out bool flipHorizontal, out bool flipVertical )
		{
			...
			// Obtained from ((Fix64)1686629713.065252369824872831112M)._rawValue
		...
		public static Fix64 Asin( Fix64 x )
		{
			x = Acos( x );
			var xl = x._rawValue;
			var rawAngle = PI_OVER_2 - xl;
			return new Fix64( rawAngle );
		}
		...
		{
			return new Fix64( value << FRACTIONAL_PLACES );
		}
		public static explicit operator int( Fix64 value )
		{
			return ( int )( value._rawValue * DIVONE );
		}
		...
		{
			return ( long )( value._rawValue * DIVONE );
		}
		...
		{
			return value._rawValue * ( float )DIVONE;
		}
		...
		{
			return value._rawValue * ( decimal )DIVONE;
		}

		public static explicit operator Fix64( string value )
		{
			return ( Fix64 )Convert.ToSingle( value );
		}

		public override bool Equals( object obj )
		{
			return obj is Fix64 fix64 && fix64._rawValue == this._rawValue;
		}
		...
		/// </summary>
		public long RawValue => this._rawValue;
